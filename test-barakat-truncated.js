...
async function testBarakatTruncated() {
  ...
  workbook.worksheets.forEach((ws, sIdx) => {
    ...
    const isLargeSheet = ws.name === 'Entries data';
    const rowLimit = isLargeSheet ? 100 : 1000; // truncate Entries data to 100 rows for fast calculation test

    ws.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      if (rowNumber > rowLimit) return; // skip rows > rowLimit
  ...
  const res = await page.evaluate(async (data) => {
    ...
    console.log('Waiting 3 seconds for truncated workbook formula calculation...');
    ...
    return {
      Summary_A1_value: sSummary ? sSummary.getRange(0, 0, 1, 1).getValue() : null,
      EntriesData_A2_value: sEntries ? sEntries.getRange(1, 0, 1, 1).getValue() : null,
      SummaryHadiah_A1_imported: { val: rangeA1.getValue(), formula: rangeA1.getFormula ? rangeA1.getFormula() : null },
      SummaryHadiah_B1_manual_cross_entries: { val: rangeB1.getValue(), formula: rangeB1.getFormula ? rangeB1.getFormula() : null },
      SummaryHadiah_C1_manual_cross_summary: { val: rangeC1.getValue(), formula: rangeC1.getFormula ? rangeC1.getFormula() : null }
    };
  }, fullWbData);

  console.log('TRUNCATED BARAKAT TEST RESULTS:');
  console.log(JSON.stringify(res, null, 2));

  fs.writeFileSync('truncated-results.json', JSON.stringify(res, null, 2));

  await browser.close();
}

testBarakatTruncated().catch(console.error);
