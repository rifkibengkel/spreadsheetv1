...
async function compareMinimalVsBarakat() {
  ...
  const res = await page.evaluate(async (data) => {
    ...
    console.log('Creating full workbook from barakat data...');
    const wb = api.createUniverSheet(data);

    console.log('Waiting 3 seconds for formula calculation...');
    await new Promise(r => setTimeout(r, 3000));

    const sSummary = wb.getSheetByName('Summary');
    const sHadiah = wb.getSheetByName('Summary Hadiah');

    const sumA1 = sSummary ? sSummary.getRange(0, 0, 1, 1).getValue() : null;
    const hadA1 = sHadiah ? sHadiah.getRange(0, 0, 1, 1).getValue() : null;
    ...
    return {
      sumA1,
      hadA1,
      summaryA1Snap: snap.sheets['Summary']?.cellData?.[0]?.[0],
      hadiahA1Snap: snap.sheets['Summary Hadiah']?.cellData?.[0]?.[0]
    };
  }, fullWbData);
...
compareMinimalVsBarakat().catch(console.error);
