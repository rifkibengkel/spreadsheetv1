...
async function debugResolver() {
  ...
  const logs = [];
  logs.push(`Summary cell 0,0: ${JSON.stringify(sheetsData['Summary']?.cellData?.[0]?.[0])}`);
  logs.push(`Summary Hadiah cell 0,0: ${JSON.stringify(sheetsData['Summary Hadiah']?.cellData?.[0]?.[0])}`);

  const cell = sheetsData['Summary Hadiah']?.cellData?.[0]?.[0];
  if (cell) {
    logs.push(`Cell f: ${cell.f}`);
    let match = cell.f.match(/^=('([^']+)'|([A-Za-z0-9_\s]+))!([A-Za-z]+)([0-9]+)$/i);
    logs.push(`Match result: ${JSON.stringify(match)}`);
    ...
    logs.push(`Target: sheet=${targetSheetName}, col=${colStr}, row=${rowNum}`);
    ...
    if (targetSheetName && colStr && rowNum) {
      ...
      logs.push(`Target cell in ${targetSheetName} R${targetRow}C${targetCol}: ${JSON.stringify(targetSheet?.cellData?.[targetRow]?.[targetCol])}`);
    }
  }

  fs.writeFileSync('debug-resolver.txt', logs.join('\n'));
  console.log('Saved debug-resolver.txt');
}

debugResolver().catch(console.error);
