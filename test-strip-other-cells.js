...
async function testStripOtherCells() {
  ...
  // TEST CASE A: Original barakat sheetsData
  // TEST CASE B: Strip all cells from Summary Hadiah except cell 0,0 ({ f: '=Summary!A1' })
  const testB_sheetsData = JSON.parse(JSON.stringify(sheetsData));
  testB_sheetsData['Summary Hadiah'].cellData = {
    0: { 0: { f: '=Summary!A1' } }
  };
  ...
  console.log('Testing Test Case B (Only cell 0,0 in Summary Hadiah)...');
  const resB = await page.evaluate(async (data) => {
  ...
  }, { id: `tb-${Date.now()}`, name: 'Test B', sheetOrder, appVersion: '3.0.0-alpha', sheets: testB_sheetsData });

  console.log('TEST CASE B RESULT (Only cell 0,0 in Summary Hadiah):');
  console.log(JSON.stringify(resB, null, 2));

  await browser.close();
}

testStripOtherCells().catch(console.error);
