...
async function testWaitFormula() {
  ...
  const res = await page.evaluate(async (data) => {
    ...
    console.log('Checking Summary Hadiah A1 immediately and at intervals...');
    ...
    for (let t = 0; t <= 10; t += 2) {
      ...
      snapshots.push({ tSec: t, val, snapCell });
    }

    return snapshots;
  }, fullWbData);

  console.log('TIMED SNAPSHOTS FOR SUMMARY HADIAH A1:');
  console.log(JSON.stringify(res, null, 2));

  await browser.close();
}

testWaitFormula().catch(console.error);
