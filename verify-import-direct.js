...
async function verifyImportDirect() {
  console.log('=====================================================================');
  console.log('DIRECT REAL FILE IMPORT VERIFICATION (report barakat.xlsx)');
  console.log('=====================================================================');
  ...
  // Perform pre-resolution for scalar formulas
  ...
  const workbookData = {
    id: `direct-verification-${Date.now()}`,
  ...
  const targets = [
    { name: 'A. Simple Same-sheet (Summary!I9)', sheet: 'Summary', row: 8, col: 8 },
    { name: 'B. Simple Cross-sheet (Summary Hadiah!A1)', sheet: 'Summary Hadiah', row: 0, col: 0 },
    { name: 'C. Large Cross-sheet (Summary Hadiah!C4)', sheet: 'Summary Hadiah', row: 3, col: 2 },
    { name: 'D. Shared Formula Follower (Summary Hadiah!C5)', sheet: 'Summary Hadiah', row: 4, col: 2 }
  ];
  ...
  const browserEval = await page.evaluate(async (wbData, targetCells) => {
    ...
    const results = targetCells.map(t => {
  ...
  fs.writeFileSync('c:\\Users\\BKKI-1\\.gemini\\antigravity\\scratch\\sheet-V-2\\direct-verification-output.json', JSON.stringify({
    totalFormulas,
    expensiveCount,
    browserEval
  }, null, 2));

  console.log('Saved direct verification results to direct-verification-output.json!');
  await browser.close();
}

verifyImportDirect().catch(console.error);
